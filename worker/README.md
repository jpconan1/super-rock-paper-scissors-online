...
2. Create the shared database with `npx wrangler d1 create super-rps-online-v1`.
...
That command builds the browser app, then deploys its static assets and the API/WebSocket Worker together. The rebuild stays on its `workers.dev` address until it is ready to replace `rps.jpconan.ca`. The ABM demo remains isolated on the `abm-demo` branch and its existing Worker.
